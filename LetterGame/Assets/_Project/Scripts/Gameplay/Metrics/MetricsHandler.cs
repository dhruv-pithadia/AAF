...
namespace LetterQuest.Gameplay.Metrics
{
    public class MetricsHandler
    {
        ...
        public void IncrementCorrectGrabs() => _data.NumPerfectGrabs++;
        ...
        public void CalculatePlayDuration(float time) => _data.PlayDuration = CalculateElapsedTimeInt(time);

        private static float CalculateElapsedTime(float time) => Time.realtimeSinceStartup - time;
        private static int CalculateElapsedTimeInt(float time) => Mathf.FloorToInt(CalculateElapsedTime(time));
    }
}
